...
> [!IMPORTANT]
> Проект переехал в [openlibrecommunity/rewl](https://github.com/openlibrecommunity/rewl). Этот репозиторий заархивирован и больше не поддерживается.

# Archivef

Репо временно или нет закрыт, почему? потому что теперь у меня нету симки через которую сканить инет
